import argparse, json, os, os.path as osp, re, shutil, subprocess, threading
from typing import Optional, Tuple

from ai_scientist.generate_ideas import search_for_papers
from ai_scientist.llm import get_response_from_llm, extract_json_between_markers, create_client, AVAILABLE_LLMS

# Hard timeout for coder.run() to prevent infinite refinement loops
_AIDER_TIMEOUT = 600  # 10 min
_ORIGINAL_CODER_RUN = None


def _patch_coder_run(coder):
    """Replace coder.run() with a timeout-guarded wrapper (10 min max)."""
    global _ORIGINAL_CODER_RUN
    if _ORIGINAL_CODER_RUN is None:
        _ORIGINAL_CODER_RUN = type(coder).run

    def _timeout_run(self, with_message=None, preproc=True):
        result = {"text": None}
        def _go():
            try:
                result["text"] = _ORIGINAL_CODER_RUN(self, with_message=with_message, preproc=preproc)
            except Exception as e:
                print(f"[AIDER ERROR] {e}")
        t = threading.Thread(target=_go, daemon=True)
        t.start()
        t.join(timeout=_AIDER_TIMEOUT)
        if t.is_alive():
            print(f"[AIDER TIMEOUT] coder.run() exceeded {_AIDER_TIMEOUT}s — using partial edits.")
        return result["text"]
    coder.run = _timeout_run.__get__(coder, type(coder))


# GENERATE LATEX
def generate_latex(coder, folder_name, pdf_file, timeout=30, num_error_corrections=2):
...
# Systematic methodology writing guidance — injected when triz_mode is active
# NOTE: Guides the LLM to apply structured thinking patterns WITHOUT using TRIZ terminology.
# The paper should read like a standard ML paper with rigorous methodology.
_SYSTEMATIC_WRITING_GUIDE = """
SYSTEMATIC METHODOLOGY — Apply these thinking patterns in your writing:

1. FRAME THE PROBLEM THROUGH TRADE-OFF ANALYSIS:
   - In the Introduction, explicitly identify the key trade-off: "We observe that
     improving [desired property] typically comes at the cost of [undesired side effect]."
   - This establishes the research gap and motivates the need for a new approach.

2. JUSTIFY THE METHOD THROUGH STRUCTURED PROBLEM-SOLVING:
   - In the Method section, explain your design choices as systematic resolutions
     of the identified trade-offs. For each component, answer:
     (a) Which specific limitation does it address?
     (b) Why is the chosen approach more effective than alternatives?
     (c) How does it complement other components of the method?
   - Present the method as a coherent, principled solution — not an ad-hoc collection of tricks.

3. EVALUATE USING MULTI-DIMENSIONAL METRICS:
   - In Results/Discussion, evaluate your method holistically:
     Benefits achieved (performance gains, safety improvements) vs.
     Costs incurred (compute overhead, complexity, data requirements) vs.
     Potential risks (overfitting, fairness concerns, failure modes).
   - Quantify each dimension with experimental evidence.

4. MAINTAIN RIGOROUS ACADEMIC STANDARDS:
   - Ground every claim in experimental results or cited literature.
   - Discuss limitations honestly and suggest concrete future work.
   - Avoid overclaiming — state precisely what was demonstrated and under what conditions.
"""

_systematic_section_boost = {
    "Abstract": "\nGUIDANCE: Frame your abstract around the key trade-off you addressed "
                "and the systematic approach you developed to resolve it. State the "
                "problem, your insight, the solution, and the evidence.\n",
    "Introduction": "\nGUIDANCE: Open by establishing the key trade-off in the domain. "
                    "Motivate your approach as a principled resolution. End with clearly "
                    "listed contributions that map to specific aspects of the trade-off.\n",
    "Method": "\nGUIDANCE: For each component of your method, clearly explain the design "
              "rationale — what specific problem does it solve and why is this approach "
              "effective? Show how components work together as a coherent system.\n",
    "Results": "\nGUIDANCE: Include a balanced assessment: What benefits did you achieve? "
               "What costs were incurred? What are the limitations? Support every claim "
               "with specific experimental results. Compare fairly against baselines.\n",
    "Conclusion": "\nGUIDANCE: Summarize the trade-off resolved and the key innovations. "
                  "Honestly discuss limitations and suggest concrete directions for "
                  "future work that build on your findings.\n",
}
...
def perform_writeup(
        ...
        engine="semanticscholar", triz_mode=None,
):
    # Install timeout guard on coder.run() to prevent infinite refinement
    _patch_coder_run(coder)

    # Systematic methodology section tips override
    triz_active = triz_mode in ("inject", "tools")
    if triz_active:
        print("[Systematic] Injecting systematic methodology into paper writing prompts.")

    def _tip(section):
        """Get tip for a section, with optional systematic methodology boost."""
        base = per_section_tips.get(section, "")
        if triz_active:
            boost = _systematic_section_boost.get(section, "")
            return boost + "\n" + base
        return base
...
IMPORTANT — Figure Availability:
A method overview figure `method_figure.png` may have been auto-generated and placed in the
project directory. If it exists, reference it in the Introduction or Method section with
an appropriate \\includegraphics command and a descriptive caption explaining the framework.
Check `notes.txt` for any generated figure descriptions.
...
IMPORTANT — Method Figure: If `method_figure.png` exists in the project directory, include it with
a proper \\begin{{figure}}...\\end{{figure}} block and a descriptive caption. The best location is
at the beginning of the Method section or at the end of the Introduction. The caption should
describe the overall framework: the problem, the proposed approach, and how components interact.
Do NOT include this figure in multiple sections — place it once at the most appropriate location.
    ...
    for section in [
        ...
        coder_out = coder.run(
            second_refinement_prompt.format(
                section=section, tips=_tip(section)
            )
            .replace(r"{{", "{")
            .replace(r"}}", "}")
        )
