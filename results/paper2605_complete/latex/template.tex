...
Prior attempts to generate counterfactuals (e.g., Wachter et al., 2017) have relied on black‑box search procedures, expensive post‑hoc optimization, or special surrogate models, all of which add computational overhead or sacrifice predictive performance.
...
We conduct experiments on a real‑world tabular dataset that represents a binary classification task (telecom customer churn, 5,643 instances, 19 features after removing identifier columns). The dataset is provided as a CSV file (data.csv) and contains a mixture of numeric and categorical features. Columns that function as row identifiers (e.g., \texttt{customerID}, \texttt{id}) are removed, categorical variables are label‑encoded, and all features are standardised to zero mean and unit variance. The instances are split into training (72\%), validation (8\%) and held‑out test (20\%) sets, preserving the original class distribution. All splits use a fixed random seed (seed~42) to ensure reproducibility across runs. The 100\% counterfactual validity observed across all models suggests that the task is relatively easy to flip; caution is advised when interpreting small proximity improvements.
...
Figure~\ref{fig:comp} presents the full set of metrics across all runs, confirming the trends described above and providing an at‑a‑glance comparison of validation loss, counterfactual quality, and random‑noise performance.  The curvature regularizer entails a per‑batch calculation of input gradients and their second‑order derivatives; Table~\ref{tab:results} shows that this increases training time from 1.4\,s to 2.2\,s ($\times 1.57$) for our setting.  The current evaluation does not include a detailed table of per‑sample metrics (e.g., sparsity distributions, iteration counts) or compare to established counterfactual generation algorithms; such a comprehensive analysis is planned for future work.  A detailed complexity analysis and study of alternative approximations is left for future work.
...
\label{sec:related}
Counterfactual explanations originated with the work of Wachter et al.~(2017), which formulates the search as a constrained optimisation problem.  Subsequent methods include DiCE (Mothilal et al., 2020), which introduces diversity metrics, and Growing Spheres (Laugel et al., 2018).  These post‑hoc approaches treat the model as fixed and can suffer from high‑curvature decision boundaries.  Input‑gradient regularisation has been used for interpretability (Smilkov et al., 2017; Ross and Doshi‑Velez, 2018) and adversarial robustness, but rarely targets counterfactual generation.  Lipschitz‑constrained networks (Miyato et al., 2018; Sokolić et al., 2017) impose a global bound on the input–output Jacobian, often at the expense of accuracy.  Our method bridges these areas by explicitly penalising the local gradient of the loss during training, thereby enhancing gradient‑based counterfactual search without harming predictive performance.

\section{Discussion}
\label{sec:discussion}
Our current evaluation is limited to a single tabular dataset and two simple baselines; no comparison is made with established counterfactual generation methods (e.g., DiCE, CEM) or with alternative regularisation techniques like Lipschitz regularisation or adversarial training.  The 2.3\% relative reduction in perturbation distance is modest and, without comparisons to other smoothness techniques (e.g., Lipschitz regularisation, adversarially robust training) or to alternative counterfactual search algorithms, it is difficult to assess whether the improvement is practically meaningful or merely noise.  Future work will incorporate at least three publicly available benchmark datasets (e.g., German Credit, Adult, COMPAS) and compare with established counterfactual generators such as Wachter et al., DiCE, and Growing Spheres, as well as with alternative smoothness‑inducing training strategies (adversarial training, Lipschitz regularization, gradient clipping).  The study is further limited by the use of a single dataset, which provides no evidence of generalizability across diverse data types or problem scales.

While penalising the squared norm of the input gradient is a known technique for stability (double backpropagation, contractive penalties), its specific application to counterfactual generation via gradient‑based inversion exploits a locally linear decision surface, which previous works have not targeted.  This regulariser is more suitable for counterfactual generation than a direct penalty on output‑logit gradients because it directly shapes the decision boundary near training points, thus improving the convergence of gradient‑based inversion.
